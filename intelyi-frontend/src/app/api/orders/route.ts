...
export async function GET(request: Request) {
  ...
  const targetUrl = queryString
    ? `${getFastApiBaseUrl()}/orders?${queryString}`
    : `${getFastApiBaseUrl()}/orders`;
...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${getFastApiBaseUrl()}/orders`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    return NextResponse.json(data ?? { error: "Failed to create order" }, { status: response.status });
  }
